...
def _is_xl(model_name):
    ...
    try:
        ...
        print(f"Could not inspect model '{model_name}': {e}. Assuming SD 1.5.")
        return False
    return False
...
def get_pipeline_for_model(model_name):
    ...
    if _is_xl(model_name):
        print(f"Model '{model_name}' correctly detected as SDXL.")
        return SDXLPipeline(model_path)
    else:
        print(f"Model '{model_name}' correctly detected as SD 1.5.")
